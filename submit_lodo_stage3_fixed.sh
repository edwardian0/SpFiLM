...
# Submit one run:
#   sbatch /users/k23123868/edward/spfilm/submit_lodo_stage3_fixed.sh \
#     refuge_zeiss 42
# Smoke one run:
#   sbatch --time=0-00:20:00 \
#     /users/k23123868/edward/spfilm/submit_lodo_stage3_fixed.sh \
...
CONFIG="$CODE_ROOT/configs/stage3_lodo_fixed_create.json"
...
OUT_DIR="$CODE_ROOT/artifacts/runs/fixed_s3_${HELD_OUT_DOMAIN}_seed_${RUN_SEED}_${SLURM_JOB_ID}"
...
cd "$CODE_ROOT"
echo "[$(date -u +%FT%TZ)] starting fixed_s3 on $(hostname) (job $SLURM_JOB_ID)"
...
echo "[$(date -u +%FT%TZ)] fixed_s3 finished"
