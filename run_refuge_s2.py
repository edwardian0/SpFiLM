#!/usr/bin/env python3
"""Step 2: train and test the plain U-Net backbone in-domain on REFUGE (Zeiss).

Train and test come from the same source: REFUGE Training400, camera domain
``refuge_zeiss``, split by a seeded stratified partition. The whole fundus is
aspect-preserving letterboxed to a square canvas; nothing is cropped. No SpFiLM,
no Global FiLM, no conditioning of any kind.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Step 2 in-domain baseline: train and test PlainUNet on REFUGE "
            "(refuge_zeiss), full-image letterboxed, no conditioning"
        )
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default artifacts/runs/refuge_s2_<timestamp>)"
        ),
    )
...
def print_header(config: Stage2Config, device: torch.device, run_dir: Path, smoke: bool) -> None:
    print("=" * 96, flush=True)
    print("SpFiLM Step 2 | in-domain plain U-Net baseline | REFUGE Training400", flush=True)
    print("=" * 96, flush=True)
    for label, value in (
    ...
    print("-" * 96, flush=True)
    for note in (CONDITIONING_NOTE, INPUT_POLICY_NOTE, DEEP_SUPERVISION_NOTE):
