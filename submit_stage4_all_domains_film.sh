...
#SBATCH --output=/users/k23123868/edward/logs/allf_s4_%j.out
#SBATCH --error=/users/k23123868/edward/logs/allf_s4_%j.err
#SBATCH --constraint="a100|a40|a30|l40s|h100"
#SBATCH --exclude=erc-hpc-comp[048,050,054,170-175,177,178,196,235-239,242,252,253]
#
# Step 4 train-on-all, Global FiLM arm: one model per seed trained on the pooled
# budgeted train partitions of the three active domains (120 train / 30 val,
# RIM-ONE-DL inactive), scored on each domain's own 50 test images with its own
# known code (codes 0/1/2) plus a fixed-code sweep under every other code.
# Paired with submit_stage4_all_domains_plain.sh on identical test sets.
# Submit one seed:
#   sbatch /users/k23123868/edward/spfilm/submit_stage4_all_domains_film.sh 42
# Smoke one seed:
#   sbatch --time=0-00:20:00 /users/k23123868/edward/spfilm/submit_stage4_all_domains_film.sh 42 --smoke
# The full protocol is 5 seeds x 2 arms = 10 independent submissions.

set -euo pipefail

CODE_ROOT="/users/k23123868/edward/spfilm"
CONFIG="$CODE_ROOT/configs/stage4_all_domains_global_film_3dom_create.json"
...
OUT_DIR="$CODE_ROOT/artifacts/runs/allf_s4_seed_${RUN_SEED}_${SLURM_JOB_ID}"
...
cd "$CODE_ROOT"
echo "[$(date -u +%FT%TZ)] starting allf_s4 on $(hostname) (job $SLURM_JOB_ID)"
...
echo "[$(date -u +%FT%TZ)] allf_s4 finished"
