...
#SBATCH --export=NONE
#SBATCH --output=/users/k23123868/edward/logs/plain_s5_%j.out
#SBATCH --error=/users/k23123868/edward/logs/plain_s5_%j.err
...
# on two, test on the held-out third, 80 train / 20 val / 50 test. Plain U-Net
# arm, run by run_stage5_lodo.py. Paired with submit_stage5_lodo_global_film.sh
# on identical test images: same folds, seeds and schedule, only the config's
# arm differs. The Step 4 LODO scripts (submit_stage4_plain.sh /
# submit_stage4_global_film.sh) define the same comparison; launching both sets
# would duplicate the 30 jobs. Train-on-all FiLM runs (120 train) finished in
# ~20 min and plain is lighter, so the 3 h limit is ample; confirm from the smoke
# job's epoch_seconds.
# The full 3-domain x 5-seed arm is 15 independent submissions.
# Submit one run:
#   sbatch /users/k23123868/edward/spfilm/submit_stage5_lodo_plain.sh \
#     refuge_zeiss 42
# Smoke one run:
#   sbatch --time=0-00:20:00 \
#     /users/k23123868/edward/spfilm/submit_stage5_lodo_plain.sh \
#     refuge_zeiss 42 --smoke

set -euo pipefail

CODE_ROOT="/users/k23123868/edward/spfilm"
CONFIG="$CODE_ROOT/configs/stage5_lodo_plain_3dom_create.json"
...
OUT_DIR="$CODE_ROOT/artifacts/runs/plain_s5_${HELD_OUT_DOMAIN}_seed_${RUN_SEED}_${SLURM_JOB_ID}"
...
cd "$CODE_ROOT"
echo "[$(date -u +%FT%TZ)] starting plain_s5 on $(hostname) (job $SLURM_JOB_ID)"
...
echo "[$(date -u +%FT%TZ)] plain_s5 finished"
